...
function getStickyBucketJs(key, salt = "") {
  ...
  return {
    ...
    hashRaw: hash.toString(16).padStart(16, "0"),
  };
}
...
window.openExperimentModal = function (flagKey) {
  window.dispatchEvent(
    new CustomEvent("open-experiment-modal", { detail: { key: flagKey } }),
  );
};
...
document.addEventListener("alpine:init", () => {
  ...
  function dropdownComponent() {
    return {
      ...
      selectEnv(env) {
        if (
          window.FlaguraApp &&
          typeof window.FlaguraApp.switchEnv === "function"
        ) {
      ...
      openProfile() {
        if (
          window.FlaguraApp &&
          typeof window.FlaguraApp.navigateTo === "function"
        ) {
      ...
      selectProject(el) {
        if (
          window.FlaguraApp &&
          typeof window.FlaguraApp.switchProject === "function"
        ) {
    ...
    return {
      ...
      copyCommand() {
        const cmdMap = {
          ...
          docker:
            "docker run --rm -it ghcr.io/dhawalhost/flagura:latest flagura --help",
        };
  ...
  function flagRowComponent() {
    return {
      ...
      get isDev() {
        return !!(
          this.envs &&
          this.envs.development &&
          this.envs.development.enabled
        );
      },
      get isStg() {
        return !!(this.envs && this.envs.staging && this.envs.staging.enabled);
      },
      get isProd() {
        return !!(
          this.envs &&
          this.envs.production &&
          this.envs.production.enabled
        );
      },
      get prodPct() {
        return this.envs &&
          this.envs.production &&
          this.envs.production.percentage !== undefined
          ? this.envs.production.percentage
          : 0;
      },
      get devPct() {
        return this.envs &&
          this.envs.development &&
          this.envs.development.percentage !== undefined
          ? this.envs.development.percentage
          : 0;
      },
      get stgPct() {
        return this.envs &&
          this.envs.staging &&
          this.envs.staging.percentage !== undefined
          ? this.envs.staging.percentage
          : 0;
      },
      copyKey() {
      ...
      async toggleEnv(env) {
        ...
        if (
          window.FlaguraApp &&
          typeof window.FlaguraApp.toggleFlagEnvStatus === "function"
        ) {
          ...
          try {
            ...
            if (res.ok) {
              ...
              if (res.status === 401) {
                ...
                if (window.showToast)
                  window.showToast('Flag "' + flagKey + '" not found', "error");
              } else {
                if (window.showToast)
                  window.showToast(
            ...
            if (window.showToast)
              window.showToast("Toggle failed: " + e.message, "error");
          }
        }
        if (!ok) {
      ...
      toggleDev() {
        this.toggleEnv("development");
      },
      toggleStg() {
        this.toggleEnv("staging");
      },
      toggleProd() {
        this.toggleEnv("production");
      },
      openHygiene(el) {
      ...
      openEditFlag(el) {
        ...
        if (
          window.FlaguraApp &&
          typeof window.FlaguraApp.openEditFlagEditor === "function"
        ) {
          window.FlaguraApp.openEditFlagEditor(flagKey);
        } else {
          window.dispatchEvent(
            new CustomEvent("open-edit-flag-editor", {
              detail: { key: flagKey },
            }),
          );
        }
      ...
      deleteFlag(el) {
        ...
        if (
          window.FlaguraApp &&
          typeof window.FlaguraApp.deleteFlagRow === "function"
        ) {
    ...
    return {
      ...
      init() {
        if (
          this.orgId === "" &&
          this.$el &&
          this.$el.querySelector("select[name=orgId] option")
        ) {
          this.orgId = this.$el.querySelector(
            "select[name=orgId] option",
          ).value;
        }
      },
        ...
        try {
          const res = await fetch(`/api/v1/change-requests/${crId}/review`, {
            ...
            body: JSON.stringify({
              ...
              comments:
                this.reviewComments ||
                (approved ? "Approved for deployment" : "Rejected"),
            }),
          });
          const data = await res.json();
          if (res.ok) {
            if (approved) {
              await fetch(`/api/v1/change-requests/${crId}/apply`, {
                method: "POST",
              });
            }
            window.location.reload();
  ...
  Alpine.data("globalApp", () => ({
    ...
    selectedWallpaper:
      "https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?auto=format&fit=crop&w=2560&q=85",
    wallpapers: [
      {
        id: "slate-waves",
        name: "Sober Slate Waves",
    ...
    syncUrl(view) {
      try {
        if (window.history && window.history.pushState) {
          ...
          const targetPath =
            view === "overview" || !view ? "/dashboard" : "/dashboard/" + view;
          url.searchParams.delete("tab");
          url.searchParams.delete("view");
          const qs = url.searchParams.toString();
          const targetUrl =
            targetPath + (qs ? "?" + qs : "") + (url.hash || "");
          ...
          const docTitle =
            viewTitle === "Overview" || viewTitle === "Developer Console"
              ? "Developer Console"
              : viewTitle;
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (res.status === 401) {
            ...
            this.showToast(
              "Toggle failed: server error (HTTP " + res.status + ")",
              "error",
            );
          }
          return false;
        ...
        if (res.ok) {
          this.showToast(
            "Flag " +
              key +
              " " +
              (isEnabled ? "ENABLED" : "DISABLED (Kill-Switch)"),
          );
        }
      } catch (e) {
    ...
    async deleteFlagRow(key) {
      if (confirm("Permanently remove feature flag " + key + "?")) {
        try {
          ...
          this.showToast(
            "Network error deleting flag: " + err.message,
            "error",
          );
        }
      }
    ...
    init() {
      ...
      try {
        ...
        if (params.has("tab") || params.has("view")) {
          ...
          params.delete("view");
          const targetPath =
            this.activeView === "overview" || !this.activeView
              ? "/dashboard"
              : "/dashboard/" + this.activeView;
          const qs = params.toString();
          const cleanUrl =
            targetPath + (qs ? "?" + qs : "") + (window.location.hash || "");
        ...
        if (
          envParam &&
          ["production", "staging", "development"].includes(envParam)
        ) {
        ...
        const modalParam = (params.get("tab") || params.get("modal") || "")
          .toLowerCase()
          .trim();
        if (modalParam === "governance" || modalParam === "approvals") {
          ...
          this.$nextTick(() =>
            window.dispatchEvent(
              new CustomEvent("open-hygiene-modal", { detail: {} }),
            ),
          );
  ...
  Alpine.data("flagMatrixEnterpriseComponent", () => ({
    ...
    toggleSelectAll(checked) {
      if (checked) {
        ...
        this.selectedFlags = Array.from(rows).map((r) =>
          r.id.replace("flag-row-", ""),
        );
      } else {
        this.selectedFlags = [];
      }
    ...
    showToast(msg, type) {
      if (
        window.FlaguraApp &&
        typeof window.FlaguraApp.showToast === "function"
      ) {
    ...
    async bulkToggleEnv(env, enabled) {
      ...
      this.showToast(
        "Updated " + this.selectedFlags.length + " flags in " + env,
      );
    ...
    matchesEnterpriseFilter(key, name, tags, isProd, prodPct, healthStatus) {
      ...
      if (this.envFilter === "canary" && (!isProd || prodPct >= 100))
        return false;

      if (this.selectedTag !== "all" && !tags.includes(this.selectedTag))
        return false;

      // Health Status Filter: 'all', 'stale', 'dead', 'active'
      if (this.healthFilter === "stale" && healthStatus !== "READY_FOR_CLEANUP")
        return false;
      if (this.healthFilter === "dead" && healthStatus !== "DEAD_FLAG")
        return false;
      if (this.healthFilter === "active" && healthStatus !== "ACTIVE")
        return false;
    ...
    matchesRow(el, isProd, prodPct) {
      ...
      return this.matchesEnterpriseFilter(
        key,
        name,
        tags,
        isProd,
  ...
  Alpine.data("flagMatrixViewComponent", () => ({
    ...
    matchesFilter(key, name, tags, isEnabled, rulesCount, strategy) {
      ...
      if (
        this.statusFilter === "rollout" &&
        (!isEnabled || strategy !== "percentage")
      )
        return false;
      if (this.statusFilter === "rules" && (!isEnabled || rulesCount === 0))
        return false;
  ...
  Alpine.data("liveEvaluatorComponent", () => ({
    ...
    async evaluateNow() {
      ...
      try {
        let parsedCustom = {};
        try {
          if (this.customJson.trim())
            parsedCustom = JSON.parse(this.customJson);
        } catch (e) {}
        const flagsList =
          this.selectedFlagKey === "all" ? [] : [this.selectedFlagKey];
    ...
    getCurlCommand() {
      const flagsArg =
        this.selectedFlagKey === "all"
          ? '["*"]'
          : `["${this.selectedFlagKey}"]`;
  ...
  Alpine.data("analyticsViewComponent", () => ({
    ...
    async renderChart() {
      ...
      let points = [
        14200, 11500, 9800, 18400, 42100, 78900, 114000, 128500, 134200, 119000,
        89400, 48200,
      ];
      try {
        const res = await fetch(
          "/api/v1/telemetry/stats?flag=" +
            encodeURIComponent(this.selectedAnalyticsFlag),
        );
        if (res.ok) {
  ...
  Alpine.data("benchmarkViewComponent", () => ({
    ...
    getSpeedupFactor() {
      ...
      return (
        Math.round((this.networkResults.avg * 1000000) / 85).toLocaleString() +
        "x"
      );
    },
    init() {
    ...
    async runStressTest() {
      ...
      try {
        ...
        if (window.showToast)
          window.showToast("Benchmark failed: " + e.message, "error");
      } finally {
    ...
    async probeLiveNetwork() {
      ...
      try {
        ...
        this.showToast(
          "Network probe complete: avg " +
            this.networkResults.avg.toFixed(1) +
            "ms",
        );
      } catch (e) {
  ...
  Alpine.data("auditViewComponent", () => ({
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (this.integrityResult.valid) {
            this.showToast(
              `Audit chain verified: ${this.integrityResult.total_verified} chained records intact`,
              "success",
            );
          } else {
            this.showToast(
              `Tampering detected at ${this.integrityResult.broken_entry_id || "entry"}!`,
              "error",
            );
          }
        } else {
    ...
    exportLogs(format = "csv") {
      ...
      this.showToast(
        `Exporting audit trail as ${format.toUpperCase()}...`,
        "info",
      );
    },
  }));
  ...
  Alpine.data("sdkViewComponent", () => ({
    ...
    getSdkSnippet() {
      ...
      if (this.sdkTab === "go") {
        ...
        const selfHostEnv =
          window.location.origin !== "https://flagura.dev"
            ? `export FLAGURA_ENDPOINT="${window.location.origin}"\n`
            : "";
  ...
  Alpine.data("flagEditorComponent", () => ({
          ...
          try {
            ...
            if (res.ok) {
              ...
              if (flag) {
                ...
                this.form.environments = JSON.parse(
                ...
                for (const env of ["production", "staging", "development"]) {
                  ...
                  for (const r of this.form.environments[env].rules) {
                    if (!r.valuesInput && r.values) {
                      r.valuesInput = Array.isArray(r.values)
                        ? r.values.join(", ")
                        : String(r.values);
                    }
                  }
    ...
    addRule() {
      this.form.environments[this.envTab].rules.push({
        ...
        name:
          "Custom Rule #" +
          (this.form.environments[this.envTab].rules.length + 1),
        attribute: "email",
        operator: "ends_with",
    ...
    syncEnvConfigToAll() {
      const cfg = JSON.parse(
        JSON.stringify(this.form.environments[this.envTab]),
      );
      ...
      this.showToast(
        "Synchronized " + this.envTab + " config to all environments",
      );
    },
    async promoteEnvironment(from, to) {
      if (
        !confirm(
          `Are you sure you want to promote ${from} rules directly to ${to}? This will overwrite ${to} configuration.`,
        )
      )
        return;
      try {
        const res = await fetch(
          `/api/v1/flags/${this.form.key}/promote?from=${from}&to=${to}`,
          {
            method: "POST",
            credentials: "same-origin",
          },
        ...
        if (res.ok) {
          this.showToast(
            `Successfully promoted ${this.form.key} from ${from} to ${to}!`,
          );
  ...
  Alpine.data("landingPageComponent", () => ({
    ...
    copySnippet() {
      ...
      if (this.sdkLanguage === "go") {
        ...
        code =
          'curl -X POST https://api.flagura.dev/api/v1/evaluate -H "Content-Type: application/json" -H "Authorization: Bearer flg_live_secret" -d \'{"flags":["ai-smart-search"],"context":{"user_id":"usr_alex_42","email":"alex@company.com"}}\'';
      }
    ...
    async runBrowserBenchmark(isSilent = false) {
      ...
      if (!isSilent) {
        this.showToast(
          "Measured 100,000 evaluations in " + durStr + "ms (" + opsStr + ")",
        );
      }
    },
  }));
  ...
  Alpine.data("authPageComponent", () => ({
    ...
    getSubtitleText() {
      if (this.mode === "signup")
        return "Get started with multi-tenant flag management";
      if (this.mode === "forgot")
        return "We will send you a secure recovery link";
      if (this.mode === "reset") return "Choose a strong password to continue";
      if (this.mode === "sso")
        return "Sign in with your organization identity provider";
      return "Access your team feature flags, rollouts, and environments";
    },

    isMainAuthMode() {
      return (
        this.mode === "signin" || this.mode === "signup" || this.mode === "sso"
      );
    },

    initAuth() {
      // Query enabled OAuth providers
      ...
      if (errorParam) {
        if (errorParam === "domain_not_found") {
          this.error =
            "No Single Sign-On (OIDC) configuration found for this domain.";
          this.mode = "sso";
        } else if (errorParam === "domain_not_allowed") {
          this.error =
            "Your email address domain is not authorized for SSO access to this organization.";
          this.mode = "sso";
        } else if (errorParam === "csrf_state_invalid") {
          this.error =
            "Single Sign-On session verification failed (CSRF state mismatch or expired). Please try again.";
          this.mode = "sso";
        } else {
          ...
          .then((data) => {
            if (data.invitation) {
              ...
              if (data.invitation.email)
                this.signUpForm.email = data.invitation.email;
              if (data.invitation.role)
                this.signUpForm.role = data.invitation.role;
              this.successMessage =
    ...
    evaluatePassword(pwd) {
      ...
      this.criteria.special = /[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?\x60~]/.test(
        pwd,
      );
    ...
    async submitSignUp() {
      if (!this.isPasswordValid(this.signUpForm.passphrase)) {
        this.error =
          "Password does not meet all security complexity requirements (8+ chars, uppercase, lowercase, number, and special symbol).";
        return;
      }
    ...
    async submitForgotPassword() {
      ...
      try {
        ...
        if (res.ok) {
          this.successMessage =
            data.message || "Recovery instructions sent to your email.";
    ...
    submitSSO() {
      ...
      this.error = "";
      window.location.href =
        "/api/v1/auth/oidc/login?domain=" + encodeURIComponent(raw);
    },
  }));
  ...
  Alpine.data("experimentModalComponent", () => ({
    ...
    async fetchReport() {
      ...
      try {
        const res = await fetch(
          `/api/v1/experiments/${this.flagKey}?metric=${encodeURIComponent(this.metricName)}`,
        );
        if (res.ok) {
    ...
    hasComparisons() {
      return (
        this.report && Object.keys(this.report.comparisons || {}).length > 0
      );
    },
    getWinningAction() {
      if (
        !this.report ||
        !this.report.winner_variant ||
  ...
  Alpine.data("hygieneModalComponent", () => ({
    ...
    getDiffHtml() {
      ...
      if (this.cleanupLang === "go") {
        if (this.flagStatus === "DEAD_FLAG") {
          return (
            '<span class="text-red-400 font-bold">- if client.IsEnabled(ctx, "' +
            k +
            '", userCtx) {</span>\n' +
        ...
        if (this.flagStatus === "DEAD_FLAG") {
          return (
            '<span class="text-red-400 font-bold">- if (await client.evaluate("' +
            k +
            '", user)) {</span>\n' +
        ...
        if (this.flagStatus === "DEAD_FLAG") {
          return (
            '<span class="text-red-400 font-bold">- if client.is_enabled("' +
            k +
            '", user):</span>\n' +
    ...
    async deleteFlag(key) {
      if (!confirm(`Permanently delete feature flag '${key}' from database?`))
        return;
      try {
        const res = await fetch("/api/v1/flags/" + key, {
          method: "DELETE",
  ...
  function profileSettingsComponent() {
    return {
      ...
      async fetchOIDCConfig(orgId) {
        ...
        try {
          const res = await fetch(
            "/api/v1/organizations/" + encodeURIComponent(orgId) + "/oidc",
            {
              credentials: "same-origin",
            },
          );
          ...
          if (!res.ok) {
            ...
            throw new Error(
              errData.message || "Failed to load OIDC configuration",
            );
          }
          const data = await res.json();
        ...
        try {
          ...
          const res = await fetch(
            "/api/v1/organizations/" +
              encodeURIComponent(this.selectedOrgId) +
              "/oidc",
            {
              method: "PUT",
              headers: { "Content-Type": "application/json" },
          ...
          if (!res.ok) {
            throw new Error(
              data.message || "Failed to save OIDC configuration",
            );
          }
          ...
          this.showToast(
            "SSO (OIDC) configuration saved successfully!",
            "info",
          );
        } catch (err) {
          this.showToast(err.message, "error");
      ...
      async deleteOIDCConfig() {
        ...
        if (
          !confirm(
            "Are you sure you want to disable and remove Single Sign-On for this organization? Members will no longer be able to log in via OIDC.",
          )
        ) {
          return;
        }
        ...
        try {
          ...
          if (!res.ok) {
            const errData = await res.json().catch(() => ({}));
            throw new Error(
              errData.message || "Failed to delete OIDC configuration",
            );
          }
          this.oidcConfig = null;
          this.oidcForm = {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (dot && ring) {
    ...
    document
      .querySelectorAll(
        'button, a, input, select, textarea, .clickable, [role="button"]',
      )
      .forEach((el) => {
  ...
  if (canvas && window.THREE) {
    ...
    const animate = () => {
      ...
      particlesMesh.rotation.y +=
        (mouseTargetX * 0.5 - particlesMesh.rotation.y) * 0.03;
      particlesMesh.rotation.x +=
        (-mouseTargetY * 0.5 - particlesMesh.rotation.x) * 0.03;
