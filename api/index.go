...
func initServer() {
	...
	if dbURL != "" {
		...
		if err != nil {
			log.Printf("[WARN] Failed to connect to Supabase PostgreSQL: %v. Falling back to in-memory store.", err)
			st = store.NewMemoryStore()
		} else {
			log.Println("[INFO] Successfully connected to Supabase PostgreSQL")
			st = pgStore
		}
	} else {
